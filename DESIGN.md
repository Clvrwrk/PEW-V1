...
Contrast verification (WCAG AA, 4.5:1 minimum normal text):
...
**Spacing scale:** 4px base unit, standard 8-point grid progression. Tokens: `xs` (4px) for micro-adjustments, `sm` (8px) for inline gaps, `md` (16px) for component padding, `lg` (24px) for card internal padding, `xl` (32px) for hero/testimonial padding, `2xl`–`5xl` for section rhythm.
...
**Content width:** 90% / 5% pattern applies to all sections. Full-bleed sections (hero backgrounds, CTA bands, dark section fills) use `w-full` on the outer element, with the 90% inner container controlling readable content width.
...
**Shadows:** Used only for modals, dropdown panels, and the office-contact card in the map component. Card-level shadows are avoided on light sections — the border ring (`ring-1 ring-border`) provides the necessary separation without shadow weight that competes with the brand's flat, direct character.
...
**DO** run `npx @google/design.md lint DESIGN.md` after any token change. It catches: broken token references, WCAG contrast violations in component definitions, missing required sections, orphaned tokens.
